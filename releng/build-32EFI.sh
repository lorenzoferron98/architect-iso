...
make_efi() {
    ...
    cp ${work_dir}/${arch}/airootfs/usr/share/efitools/efi/Loader.efi ${work_dir}/iso/EFI/boot/bootia32.efi
    cp ${work_dir}/${arch}/airootfs/usr/share/efitools/efi/HashTool.efi ${work_dir}/iso/EFI/boot/

    cp ${work_dir}/${arch}/airootfs/usr/lib/systemd/boot/efi/systemd-bootia32.efi ${work_dir}/iso/EFI/boot/loader.efi

    mkdir -p ${work_dir}/iso/loader/entries
    cp ${script_path}/efiboot/loader/loaderx86.conf ${work_dir}/iso/loader/loader.conf
    cp ${script_path}/efiboot/loader/entries/uefi-shell-v2-x86.conf ${work_dir}/iso/loader/entries/
    cp ${script_path}/efiboot/loader/entries/uefi-shell-v1-x86.conf ${work_dir}/iso/loader/entries/

    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86-usb.conf > ${work_dir}/iso/loader/entries/archiso-x86.conf

    # EFI Shell 2.0 for UEFI 2.3+ ( http://sourceforge.net/apps/mediawiki/tianocore/index.php?title=UEFI_Shell )
    curl -o ${work_dir}/iso/EFI/shellx86_v2.efi https://svn.code.sf.net/p/edk2/code/trunk/edk2/ShellBinPkg/UefiShell/Ia32/Shell.efi
    # EFI Shell 1.0 for non UEFI 2.3+ ( http://sourceforge.net/apps/mediawiki/tianocore/index.php?title=Efi-shell )
    curl -o ${work_dir}/iso/EFI/shellx86_v1.efi https://svn.code.sf.net/p/edk2/code/trunk/edk2/EdkShellBinPkg/FullShell/Ia32/Shell_Full.efi
}
...
make_efiboot() {
    ...
    cp ${work_dir}/${arch}/airootfs/usr/share/efitools/efi/Loader.efi ${work_dir}/efiboot/EFI/boot/bootia32.efi
    cp ${work_dir}/${arch}/airootfs/usr/share/efitools/efi/HashTool.efi ${work_dir}/efiboot/EFI/boot/

    cp ${work_dir}/${arch}/airootfs/usr/lib/systemd/boot/efi/systemd-bootia32.efi ${work_dir}/efiboot/EFI/boot/loader.efi

    mkdir -p ${work_dir}/efiboot/loader/entries
    cp ${script_path}/efiboot/loader/loaderx86.conf ${work_dir}/efiboot/loader/loader.conf
    cp ${script_path}/efiboot/loader/entries/uefi-shell-v2-x86.conf ${work_dir}/efiboot/loader/entries/
    cp ${script_path}/efiboot/loader/entries/uefi-shell-v1-x86.conf ${work_dir}/efiboot/loader/entries/

    sed "s|%ARCHISO_LABEL%|${iso_label}|g;
         s|%INSTALL_DIR%|${install_dir}|g" \
        ${script_path}/efiboot/loader/entries/archiso-x86-cd.conf > ${work_dir}/efiboot/loader/entries/archiso-x86.conf

    cp ${work_dir}/iso/EFI/shellx86_v2.efi ${work_dir}/efiboot/EFI/
    cp ${work_dir}/iso/EFI/shellx86_v1.efi ${work_dir}/efiboot/EFI/
...
# Build ISO
make_iso() {
    mkarchiso ${verbose} -w "${work_dir}" -D "${install_dir}" -L "${iso_label}" -o "${out_dir}" iso "${iso_name}-${iso_version}-32bit-EFI.iso"
}
